...
COLORS = {
    "bg_dark": "#0d0d1a",        # Deep dark background
    "bg_medium": "#141428",       # Slightly lighter
    "bg_light": "#1e1e3c",        # Cards hover
    "accent": "#6366f1",          # Modern indigo
    "accent_hover": "#818cf8",    # Lighter indigo
    "accent_secondary": "#ec4899", # Pink accent
    "success": "#10b981",         # Emerald green
    "warning": "#f59e0b",         # Amber
    "error": "#ef4444",           # Red
    "text_primary": "#f8fafc",    # Almost white
    "text_secondary": "#94a3b8",  # Slate gray
    "border": "#2e2e5a",          # Subtle border
    "card_bg": "#1a1a36",         # Card background
    "online_dot": "#10b981",      # Green dot
    "selected": "#2d2d5c",        # Selected state
    "gradient_start": "#6366f1",  # For gradient effects
    "gradient_end": "#ec4899",    # Pink end
}

# Light theme colors - Modern & Clean
COLORS_LIGHT = {
    "bg_dark": "#f8fafc",         # Soft white
    "bg_medium": "#f1f5f9",       # Light gray
    "bg_light": "#e2e8f0",        # Hover state
    ...
    "online_dot": "#10b981",      # Green
    "selected": "#e0e7ff",        # Light indigo
...
class UserFrame(ctk.CTkFrame):
    """Modern user card with avatar, name, and status"""
    ...
    def setup_ui(self):
        ...
        self.avatar_container = ctk.CTkFrame(self, fg_color="transparent", width=56, height=56)
        self.avatar_container.grid(row=0, column=0, padx=(14, 10), pady=14)
        self.avatar_container.grid_propagate(False)
       
        # Avatar frame with modern gradient-like effect
        self.avatar_size = 48
        ...
        # Online indicator dot - modern pulsing style
        self.online_dot = ctk.CTkFrame(
            ...
            border_width=3,
            border_color=COLORS["bg_medium"]
        )
        self.online_dot.place(relx=1.0, rely=1.0, anchor="se", x=0, y=0)
        
        # Username frame with edit option for self
        self.name_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.name_frame.grid(row=0, column=1, sticky="ew", padx=(0, 14))
        ...
        self.name_label = ctk.CTkLabel(
            self.name_frame,
            text=self.username,
            font=("SF Pro Display", 15, "bold") if platform.system() == "Darwin" else ("Segoe UI", 15, "bold"),
            text_color=COLORS["text_primary"],
            anchor="w"
        )
        self.name_label.grid(row=0, column=0, sticky="w", pady=(0, 2))
        
        # Subtitle (IP or "You") with modern styling
        subtitle_text = "You • Online" if self.is_self else (self.ip_address or "Available")
        ...
        if self.is_self:
            # Edit button with modern icon style
...
class TransferItemWidget(ctk.CTkFrame):
    """Modern UI widget for a single transfer in the queue"""
    ...
    def setup_ui(self):
        ...
        # Left: Icon based on direction and status - modern circular container
        ...
        icon_container.grid(row=0, column=0, rowspan=2, padx=(14, 10), pady=14)
        icon_container.grid_propagate(False)
        
        self.icon_label = ctk.CTkLabel(
            icon_container,
            text=self._get_icon(),
            font=("SF Pro Display", 20) if platform.system() == "Darwin" else ("Segoe UI", 20)
        )
        self.icon_label.place(relx=0.5, rely=0.5, anchor="center")
        
        # Middle: File info
        info_frame = ctk.CTkFrame(self, fg_color="transparent")
        info_frame.grid(row=0, column=1, sticky="ew", padx=(0, 8), pady=(14, 0))
        info_frame.grid_columnconfigure(0, weight=1)
        
        # File name with better truncation
        self.name_label = ctk.CTkLabel(
            info_frame,
            text=self._truncate_name(self.transfer.file_name, 28),
        ...
        self.progress_frame.grid(row=1, column=1, sticky="ew", padx=(0, 8), pady=(4, 14))
        ...
        right_frame.grid(row=0, column=2, rowspan=2, padx=(0, 14), pady=14)
        
        # File size with better styling
        ...
        # Cancel button - modern circular style
        ...
        # Retry button - modern circular style
...
class SettingsDialog(ctk.CTkToplevel):
    ...
    def setup_ui(self):
        ...
        header = ctk.CTkLabel(
            header_frame,
            text="⚙️ Settings",
            font=("SF Pro Display", 26, "bold") if platform.system() == "Darwin" else ("Segoe UI", 26, "bold"),
            text_color=COLORS["text_primary"]
        )
        header.pack()
        ...
        subtitle = ctk.CTkLabel(
            header_frame,
            text="Customize your NetXend experience",
        ...
        save_frame.pack(fill="x", padx=24, pady=(20, 0))
        ...
        self.save_location_entry.pack(side="left", padx=(0, 10))
        ...
        theme_frame.pack(fill="x", padx=24, pady=(16, 0))
        ...
        cancel_btn.pack(side="left", padx=(0, 14))
...
class NetXendApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        logo_frame.grid(row=0, column=0, padx=24, pady=14)
        
        # App icon with gradient-like effect
        ...
        version_frame.pack(side="left", padx=(12, 0))
        ...
        # Header buttons with modern pill style
        header_btns = ctk.CTkFrame(self.header, fg_color="transparent")
        header_btns.grid(row=0, column=2, padx=24)
        
        # Open folder button - ghost style
        ...
        # Sync/Update button - accent style
        ...
        self.drop_zone.grid(row=0, column=0, sticky="nsew", padx=(32, 16), pady=32)
        
        # Drop zone content container
        drop_content = ctk.CTkFrame(self.drop_zone, fg_color="transparent")
        drop_content.place(relx=0.5, rely=0.5, anchor="center")
        
        # Modern upload icon with glow effect container
        icon_container = ctk.CTkFrame(
            drop_content,
            width=100,
            height=100,
        ...
        icon_container.pack(pady=(0, 24))
        icon_container.pack_propagate(False)
        
        self.upload_icon_label = ctk.CTkLabel(
            icon_container,
            text="⬆",
            font=("SF Pro Display", 42) if platform.system() == "Darwin" else ("Segoe UI", 42),
            text_color=COLORS["accent"]
        )
        self.upload_icon_label.place(relx=0.5, rely=0.5, anchor="center")
        
        # Main drop text
        self.drop_title = ctk.CTkLabel(
            drop_content,
            text="Send Files Instantly",
            font=("SF Pro Display", 24, "bold") if platform.system() == "Darwin" else ("Segoe UI", 24, "bold"),
            text_color=COLORS["text_primary"]
        )
        ...
        self.drop_subtitle.pack(pady=(0, 32))
        
        # Buttons container
        buttons_frame = ctk.CTkFrame(drop_content, fg_color="transparent")
        buttons_frame.pack()
        
        # Select files button - modern pill style
        ...
        # Select folder button - ghost style
        ...
        self.transfer_panel.grid(row=0, column=1, sticky="nsew", padx=(0, 24), pady=32)
        self.transfer_panel.configure(width=340)
        ...
        self.status_bar.grid(row=1, column=0, columnspan=2, sticky="ew", padx=32, pady=(0, 24))
        self.status_bar.grid_propagate(False)
        self.status_bar.grid_columnconfigure(0, weight=1)
        
        # Status content
        status_content = ctk.CTkFrame(self.status_bar, fg_color="transparent")
        status_content.pack(fill="both", expand=True, padx=24, pady=14)
        ...
        # Status icon with pulsing dot effect
        ...
        # Scan button in status bar - pill style
    ...
    def apply_theme(self, theme: str):
        """Apply theme (dark/light) with full UI refresh"""
        global COLORS
        if theme == "light":
        ...
        self.drop_title.configure(text_color=COLORS["text_primary"])
        self.drop_subtitle.configure(text_color=COLORS["text_secondary"])
        ...
        # Update self user frame
        if hasattr(self, 'self_user'):
            self.self_user.configure(fg_color=COLORS["card_bg"])
            self.self_user.name_label.configure(text_color=COLORS["text_primary"])
        
        # Update peer frames
        for widget in self.peers_container.winfo_children():
            if isinstance(widget, UserFrame):
                widget.configure(fg_color=COLORS["card_bg"])
                widget.name_label.configure(text_color=COLORS["text_primary"])
